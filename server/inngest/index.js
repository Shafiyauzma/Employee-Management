...
// Cron: Check attendance at 11:30 AM IST (06:00 UTC) and email asent employees
const attendanceRemainderCron = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    // Step 5: Filter absent employees (not on leave & not checkedin )
    const absentEmployees = activeEmployees.filter((emp) => !onLeaveIds.includes(emp._id) && !checkedInIds.includes())

    // step 6: send remainder emails
    if(absentEmployees.length > 0){
        await step.run("send-remainer-emails", async ()=>{
