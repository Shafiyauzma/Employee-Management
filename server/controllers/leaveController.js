    ...
    try {
        ...
        if(employee.isDeleted){
            return res.status(403).json({
                error: "Your account id deactivated. You cannot apply for leave"
            })
        }
...
export const getLeaves = async (req, res) => {
    try {
        ...
        if(isAdmin){
            ...
            const leaves = await LeaveApplication.find(where).population("employeeId").sort({ createdAt: -1 })
